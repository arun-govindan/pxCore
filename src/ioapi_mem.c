...
ZPOS64_T ZCALLBACK ftell_mem_func OF((
   voidpf opaque,
   voidpf stream));
...
voidpf ZCALLBACK fopen_mem_func (voidpf opaque, const void* filename, int mode)
{
  (void) opaque;
...
ZPOS64_T ZCALLBACK ftell_mem_func (opaque, stream)
   voidpf opaque;
   voidpf stream;
{
  (void) opaque;
  ...
  zlib_filefunc64_def* pzlib_filefunc_def;
{
  ...
  pzlib_filefunc_def->ztell64_file = ftell_mem_func;
  pzlib_filefunc_def->zseek64_file = fseek_mem_func;
