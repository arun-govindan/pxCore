...
#check for thread sanitizer report

grep -n "WARNING: ThreadSanitizer:" $TESTLOGS
retVal=$?
if [ "$retVal" -eq 0 ]
then
  if [ "$TRAVIS_PULL_REQUEST" != "false" ]
  then
    errCause="Thread Sanitizer warning detected. Check the above logs"
    echo "********************** PRINTING TEST LOG **************************"
    cat $TESTLOGS
    echo "************************** LOG ENDS *******************************"
  else
    errCause="Thread Sanitizer warning detected. Check the log file $TESTLOGS"
  fi
    checkError -1 "Thread Sanitizer warning detected" "$errCause" "Compile spark unit test with -DENABLE_THREAD_SANITIZER=ON option and test locally."
fi
